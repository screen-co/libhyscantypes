 ...
 * @HYSCAN_SOURCE_BATHYMETRY_STARBOARD_LOW: батиметрия, правый борт, низкое разрешение
 * @HYSCAN_SOURCE_BATHYMETRY_STARBOARD_HI: батиметрия, правый борт, высокое разрешение
 * @HYSCAN_SOURCE_BATHYMETRY_PORT: батиметрия, левый борт
 * @HYSCAN_SOURCE_BATHYMETRY_PORT_LOW: батиметрия, левый борт, низкое разрешение
 * @HYSCAN_SOURCE_BATHYMETRY_PORT_HI: батиметрия, левый борт, высокое разрешение
...
/**
 * HyScanAntennaOffset:
 * @starboard: смещение антенны на правый борт, м
 * @forward: смещение антенны к носу судна, м
 * @vertical: смещение антенны по вертикали вниз, м
 * @yaw: разворот антенны от продольной оси судна, десятичный градус
 * @pitch: разворот антенны от вертикальной оси судна, десятичный градус
 * @roll: разворот антенны от поперечной оси судна, десятичный градус
 *
 ...
 * центра масс судна. Для надводных судов, вертикальное смещение базовой
 * точки необходимо указывать относительно действующей ватерлинии. Для
 * подводных аппаратов можно выбрать любую базовую точку.
 *
 * Углы установки антенны указываются для вектора, перпендикулярного её
 * рабочей плоскости. Положительные значения углов отсчитываются при
 * развороте антенны вокруг соответствующей оси по часовой стрелке.
 * Для курса (yaw) знак смещение аналогичен азимуту, для дифферента
 * (pitch) положительный знак если нос выше кормы, для крена (roll)
 * положительный знак если левый борт выше правого.
 */
